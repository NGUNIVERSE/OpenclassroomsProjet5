...
public class PersonControllerTest {
	...
	public void testGetPerson() throws Exception {
		...
		mockMvc.perform(get("/person/{id}", id1)).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$.address").value("908 73rd St")).andExpect(jsonPath("$.city").value("Culver"))
				.andExpect(jsonPath("$.zip").value("97451")).andExpect(jsonPath("$.phone").value("841-874-8547"))
		...
		mockMvc.perform(get("/person/{id}", id2)).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$.address").value("112 Steppes Pl")).andExpect(jsonPath("$.city").value("Culver"))
				.andExpect(jsonPath("$.zip").value("97451")).andExpect(jsonPath("$.phone").value("841-874-8888"))
	...
	public void testGetPersons() throws Exception {
		...
		mockMvc.perform(get("/persons")).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$[0].address", is("908 73rd St"))).andExpect(jsonPath("$[0].city", is("Culver")))
				.andExpect(jsonPath("$[0].zip", is("97451"))).andExpect(jsonPath("$[0].phone", is("841-874-8547")))
				.andExpect(jsonPath("$[0].email", is("reg@email.com"))).andExpect(jsonPath("$[1].firstName", is("Ron")))
				.andExpect(jsonPath("$[1].lastName", is("Peters")))
				.andExpect(jsonPath("$[1].address", is("112 Steppes Pl")))
				.andExpect(jsonPath("$[1].city", is("Culver"))).andExpect(jsonPath("$[1].zip", is("97451")))
	...
	public void testCreatePerson() throws Exception {
		...
		mockMvc.perform(post("/person").contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)
				.content(json)).andExpect(status().isCreated()).andExpect(jsonPath("$.firstName").value("Ron"))
	...
	public void updatePersonTest() throws Exception {
		...
		mockMvc.perform(put("/person/{id}", id1).contentType(MediaType.APPLICATION_JSON)
